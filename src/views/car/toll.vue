...
<script>
import tollFee from "@/components/car/fee/toll-fee";
export default {
  components: {
    tollFee
  },
  data() {
    return {
      visible: false,
      modalInfo: { type: 2 }, //1油费2过路费3保险费4年检费5维修保养费
      current: 1,
      pageSize: 10,
      total: 0,
      loading: false,
  ...
  methods: {
    ...
    onDelete(row) {
      ...
      this.$confirm({
        title: "删除",
        content: `是否删除${row.number}的过路费用详情`,
        onOk() {
          console.log("OK");
          _this.$api.car.deleteTollCost({ id: row.id }).then(res => {
            if (res.data.state == 0) {
