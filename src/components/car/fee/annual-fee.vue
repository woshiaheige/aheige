  ...
  methods: {
    ...
    handleOk() {
      this.$refs.ruleForm.validate(valid => {
        ...
        if (this.carId) {
          this.editAssetAifCost(values);
        } else {
          this.addAssetAifCost(values);
        }
      });
    },
    editAssetAifCost(values) {
      //修改
      values.id = this.carId;
      this.$api.car.editAssetAifCost(values).then(res => {
        if (res.data.state == 0) {
    ...
    addAssetAifCost(values) {
      //新建
      this.$api.car.addAssetAifCost(values).then(res => {
        if (res.data.state == 0) {
