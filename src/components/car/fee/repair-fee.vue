  ...
  methods: {
    ...
    handleOk() {
      this.$refs.ruleForm.validate(valid => {
        ...
        if (this.carId) {
          this.editAssetMaintenanceCost(values);
        } else {
          this.addAssetMaintenanceCost(values);
        }
      });
    },
    editAssetMaintenanceCost(values) {
      //修改
      values.id = this.carId;
      this.$api.car.editAssetMaintenanceCost(values).then(res => {
        if (res.data.state == 0) {
    ...
    addAssetMaintenanceCost(values) {
      //新建
      this.$api.car.addAssetMaintenanceCost(values).then(res => {
        if (res.data.state == 0) {
