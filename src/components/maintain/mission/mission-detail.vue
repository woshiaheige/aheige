  ...
  <div>
    <a-card :bordered="false">
      ...
      <template v-if="formInline.type == 1">
        <mission-daily :execFormInline="formInline" ref="missionDaily" />
      </template>

      <!-- 突发任务 -->
      <template v-if="formInline.type == 2">
        <mission-outburst :execFormInline="formInline" ref="missionOutburst" />
      </template>
    </a-card>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.formInline.pointId = this.$route.query.pointId || "";
