    ...
    visible(newVal) {
      if (newVal) {
        ...
        if (this.detail.fileIds && this.detail.fileIds.length > 0) {
          this.detail.fileIds.forEach((item, index) => {
            this.detail.fileContentType
              .split(",")
              .forEach((typeItem, typeIndex) => {
                if (index === typeIndex) {
                  if (typeItem.indexOf("video") > -1) {
                    ...
                    this.imgArr.push(this.fileViewOnline + item);
                  }
                }
              });
          });
